...
namespace haar_c_gamma
{
  std::string haar_integer_c_gamma(int *arr, int k2pow)
  {
    ...
    inner_haar_integer_c_gamma(curnums, k2pow, listofpairs);
    string final_msg = haar_integer_c_gamma_code(listofpairs);
    return final_msg;
  }
  void inner_haar_integer_c_gamma(std::list<int>& curnums, int k2pow, std::list<std::pair<int, int>>&listofpairs)
  {
    ...
    {
      inner_haar_integer_c_gamma(znumbers, k2pow - 1, listofpairs);
    }
  }
  std::string haar_integer_c_gamma_code(std::list<std::pair<int, int>> &listofpairs)
  {
        ...
        for (std::list<std::pair<int, int>>::iterator it = listofpairs.begin(); it != listofpairs.end(); ++it)
    {
      ...
      if(first!=-1)
      {
        string c_delta_first = c_gamma_coding(first);
        code_of_nums = c_delta_first + code_of_nums ;
      }
      string c_delta_seconed = c_gamma_coding(seconed);
  ...
  std::string haar_new_trasform_c_gamma(int *arr, int k2pow)
  {
    ...
    inner_haar_new_trasform_c_gamma(curnums, k2pow, listdifrence);
    string final_massage = haar_new_trans_c_gamma_code(listdifrence);
  ...
  void inner_haar_new_trasform_c_gamma(std::list<int>& curnums, int k2pow, std::list<int>& listdifrence)
  {
    ...
    {
      inner_haar_new_trasform_c_gamma(znumbers, k2pow - 1, listdifrence);
    }
  }
